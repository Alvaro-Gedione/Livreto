...
class LayoutContainer extends pixi_js.Container {
  constructor(params = {}) {
    ...
    __publicField(this, "_visualBounds", new pixi_js.Rectangle());
    /** The scroll bounds for the container where scroll events can occur */
    __publicField(this, "_scrollBounds", new pixi_js.Rectangle());
    this.layout = layout ?? {};
   ...
   */
  _updateMask(width, height, radius = 0, borderWidth = 0, isContentBox = false) {
  ...
  _updateBackground(computedLayout) {
    ...
    if (this._isUserBackground) {
      ...
      if (backgroundColor != null) {
        ...
        background.fill({ color: 16777215, alpha: 0 });
      }
    }
  }
  /**
  ...
  _drawBackground(computedLayout) {
    var _a;
    let borderWidth = this.layout.yoga.getBorder(load.Edge.All);
    ...
    if (overflow !== "visible") {
      this._updateMask(
        ...
        borderWidth,
        boxSizing === load.BoxSizing.ContentBox
      );
  ...
  _updateScrollArea() {
    ...
    maxRight += this.layout.yoga.getComputedPadding(load.Edge.Right);
    maxBottom += this.layout.yoga.getComputedPadding(load.Edge.Bottom);
    const borderWidth = this.layout.yoga.getBorder(load.Edge.All) || 0;
