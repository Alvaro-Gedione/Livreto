import { D as provideLoadMap, G as H, J as Sn, K as P, O as Is, X as U, Y as Te, n as component$1, q as Q } from "./dist-ZrHUti23.js";
import { createModule, defineModule } from "./@rpgjs_common.js";
//#region node_modules/@rpgjs/tiledmap/dist/client/index3.js
...
var sax = {};
var require$$0 = /* @__PURE__ */ getAugmentedNamespace(/* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
	__proto__: null,
	default: {}
	...
	(function(exports) {
		...
		Buffer2.concat = function concat(list, length) {
			...
			for (i = 0; i < list.length; ++i) {
				let buf = list[i];
				if (isInstance(buf, Uint8Array)) if (pos + buf.length > buffer2.length) {
...
function requireXml2json() {
	...
	xml2json = function(xml, userOptions) {
		var options = validateOptions(userOptions), js = xml2js2(xml, options), json, parentKey = "compact" in options && options.compact ? "_parent" : "parent";
...
var _TiledParser = class _TiledParser {
	constructor(xml, filePath = "", basePath = "") {
		...
		this.transform = (obj) => {
			...
			if (objectgroup) newObj.objects = _TiledParser.toArray(objectgroup).map((object) => {
	...
	*/
	...
	static unpackTileBytes(buffer2, size) {
		...
		if (buffer2.length !== expectedCount) throw new Error("Expected " + expectedCount + " bytes of tile data; received " + buffer2.length);
		let tileIndex = 0;
		const array = [];
	...
	static decode(obj, size) {
		...
		if (encoding == "base64") return _TiledParser.unpackTileBytes(bufferExports.Buffer.from(data.trim(), "base64"), size);
	...
	parseMap() {
		...
		const recursiveObjectGroup = (obj) => {
			...
			setLayer(group2);
			setLayer(imagelayer);
			if (group2) recursiveObjectGroup(group2);
		};
		...
		const tilesets = _TiledParser.toArray(tileset).map((tileset2) => {
			return this.transform(tileset2);
		});
		const ret = {
	...
	parseTileset() {
		const { tileset } = libExports.xml2js(this.xml, { compact: true });
		const ret = {
			...
			tiles: _TiledParser.toArray(tileset.tile).map((tile) => {
				...
				if (tile.animation) ret2.animations = _TiledParser.toArray(tile.animation.frame).map(this.transform);
	...
	*/
	addTileIndex(layer, tile, tileIndex, layerIndex) {
		...
		if (!this.tilesIndex[z]) {
			const buffer2 = /* @__PURE__ */ new ArrayBuffer(layer.size * this.realAllocateMemory * 2);
...
//#endregion
//#region node_modules/@rpgjs/tiledmap/dist/client/index5.js
...
//#endregion
//#region node_modules/@rpgjs/tiledmap/dist/client/index2.js
var client = defineModule({
	componentAnimations: [],
...
//#endregion
//#region node_modules/@rpgjs/tiledmap/dist/client/index4.js
function component($$props) {
	Q($$props);
	var _a = Sn($$props)(), data = _a.data, params = _a.params;
	var map = P(data());
	var basePath = P(params().basePath);
	H(function() {
		map.set(data());
	});
	return U(Te, null, U(Is, {
		map,
		basePath,
		createLayersPerTilesZ: true,
		objectLayer: () => U(component$1)
	}));
}
//#endregion
//#region node_modules/@rpgjs/tiledmap/dist/client/index.js
var server = null;
...
//# sourceMappingURL=@rpgjs_tiledmap_client.js.map
