...
var GlTexture = require('../../../gl/texture/GlTexture.js');
var GpuTextureSystem = require('../../../gpu/texture/GpuTextureSystem.js');
...
function getPlaceholder(renderer) {
  var _a;
  if (renderer.type === types.RendererType.WEBGPU) {
...
class ExternalSource extends TextureSource.TextureSource {
  ...
  _initGpuData(resource) {
    ...
    if (renderer.gpu) {
      this._gpuData[renderer.uid] = new GpuTextureSystem.GPUTextureGpuData(resource);
    } else {
      this._gpuData[renderer.uid] = new GlTexture.GlTexture(resource);
    }
  }
  /**
