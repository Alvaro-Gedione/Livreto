...
class NodeParser {
        ...
        if (hasHash) {
            ...
            if (tokenReader.peekTokenKind() !== Token_1.TokenKind.Period) {
                ...
                if (!tokenReader.isAccumulatedSequenceEmpty()) {
                    ...
                    const explanation = StringChecks_1.StringChecks.explainIfInvalidPackageName(packageNameExcerpt.toString());
                    if (explanation) {
            ...
            if (!tokenReader.isAccumulatedSequenceEmpty()) {
                ...
                const explanation = StringChecks_1.StringChecks.explainIfInvalidImportPath(importPathExcerpt.toString(), !!packageNameExcerpt);
                if (explanation) {
    ...
    _parseMemberIdentifier(tokenReader, tokenSequenceForErrorContext, nodeForErrorContext) {
        ...
        if (tokenReader.peekTokenKind() === Token_1.TokenKind.DoubleQuote) {
            ...
            const explanation = StringChecks_1.StringChecks.explainIfInvalidUnquotedMemberIdentifier(identifier);
            if (explanation) {
    ...
    _parseMemberSelector(tokenReader, tokenSequenceForErrorContext, nodeForErrorContext) {
        ...
        const docMemberSelector = new nodes_1.DocMemberSelector({
            parsed: true,
            configuration: this._configuration,
            selectorExcerpt,
            selector
    ...
    _parseHtmlStartTag(tokenReader) {
        ...
        if (tokenReader.peekTokenKind() !== Token_1.TokenKind.GreaterThan) {
            const failure = this._createFailureForTokensSince(tokenReader, TSDocMessageId_1.TSDocMessageId.HtmlTagMissingGreaterThan, 'Expecting an attribute or ">" or "/>"', endDelimiterMarker);
    ...
    _parseHtmlEndTag(tokenReader) {
        ...
        // Read the closing ">"
        if (tokenReader.peekTokenKind() !== Token_1.TokenKind.GreaterThan) {
            const failure = this._createFailureForToken(tokenReader, TSDocMessageId_1.TSDocMessageId.HtmlTagMissingGreaterThan, 'Expecting a closing ">" for the HTML tag');
            return this._backtrackAndCreateErrorForFailure(tokenReader, marker, '', failure);
        }
        tokenReader.readToken();
        const closingDelimiterExcerpt = tokenReader.extractAccumulatedSequence();
    ...
    _parseHtmlName(tokenReader) {
        ...
        const explanation = StringChecks_1.StringChecks.explainIfInvalidHtmlName(htmlName);
        if (explanation) {
        ...
        while (!done) {
            switch (tokenReader.peekTokenKind()) {
                ...
                case Token_1.TokenKind.Newline:
                    ...
                    break;
                case Token_1.TokenKind.Backtick:
                    const failure = this._createFailureForToken(tokenReader, TSDocMessageId_1.TSDocMessageId.CodeFenceSpecifierSyntax, 'The language specifier cannot contain backtick characters');
                ...
                case Token_1.TokenKind.EndOfInput:
                    const failure2 = this._createFailureForToken(tokenReader, TSDocMessageId_1.TSDocMessageId.CodeFenceMissingDelimiter, 'Missing closing delimiter');
    ...
    _createError(tokenReader, messageId, errorMessage) {
        ...
        const docErrorText = new nodes_1.DocErrorText({
            parsed: true,
            configuration: this._configuration,
            textExcerpt,
            messageId,
    ...
    _createFailureForToken(tokenReader, failureMessageId, failureMessage, tokenMarker) {
        ...
        const tokenSequence = new TokenSequence_1.TokenSequence({
