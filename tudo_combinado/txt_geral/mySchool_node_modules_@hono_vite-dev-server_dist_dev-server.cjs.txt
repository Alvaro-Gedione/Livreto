...
const defaultOptions = {
  ...
  exclude: [
    ...
    /\?t\=\d+$/,
    /[?&]tsr-split=[^&]*(&t=[^&]*)?$/,
    // Support for TanStack Router code splitting
    /^\/favicon\.ico$/,
    /^\/static\/.+/,
    /^\/node_modules\/.*/,
    /.*\.svelte$/,
...
function devServer(options) {
  ...
  const rewriteRequestForBase = (0, import_utils.createBasePathRewriter)(options?.base ?? defaultOptions.base);
  const shouldHandlePath = (0, import_utils.createBasePathGuard)(options?.base ?? defaultOptions.base);
  let viteBaseSetByUser;
  const plugin = {
    ...
    configResolved(config) {
      ...
      const honoBaseIsSet = honoBase !== void 0;
      const viteBaseIsSet = viteBaseSetByUser !== void 0 && viteBaseSetByUser !== "/";
    ...
    configureServer: async (server) => {
      async function createMiddleware(server2) {
        return async function(req, res, next) {
          ...
          const pathname = req.url ? (0, import_utils.safeParseUrlPath)(req.url) : void 0;
          ...
          (0, import_node_server.getRequestListener)(
            async (request) => {
              ...
              if (options?.injectClientScript !== false && response.headers.get("content-type")?.match(/^text\/html/)) {
                const viteScript = import_path.default.posix.join(viteBase, "/@vite/client");
